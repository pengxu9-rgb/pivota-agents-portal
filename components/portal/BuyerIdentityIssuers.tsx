...
export default function BuyerIdentityIssuers() {
  ...
  --data '{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"create_checkout","arguments":{"meta":{"idempotency-key":"YOUR_UNIQUE_KEY"},"checkout":{"line_items":[{"item":{"id":"sig_PRODUCT_ID"},"quantity":1}]}}}}'`}
              />
            </div>
            <p className="mt-2 text-sm leading-6 text-[var(--portal-fg-muted)]">
              The token must carry <code className="rounded bg-white px-1.5 py-1 font-mono text-xs">iss</code>, <code className="rounded bg-white px-1.5 py-1 font-mono text-xs">sub</code>, <code className="rounded bg-white px-1.5 py-1 font-mono text-xs">aud</code>, <code className="rounded bg-white px-1.5 py-1 font-mono text-xs">exp</code>, <code className="rounded bg-white px-1.5 py-1 font-mono text-xs">iat</code> and a <code className="rounded bg-white px-1.5 py-1 font-mono text-xs">kid</code> present in your JWKS. Include <code className="rounded bg-white px-1.5 py-1 font-mono text-xs">email</code> + <code className="rounded bg-white px-1.5 py-1 font-mono text-xs">email_verified: true</code> to have the buyer email attested. For checkout, also put a stable per-checkout id in <code className="rounded bg-white px-1.5 py-1 font-mono text-xs">sid</code> and keep it the same from create through complete (the <code className="rounded bg-white px-1.5 py-1 font-mono text-xs">Mcp-Session-Id</code> header is not read on this path), and send a client-generated <code className="rounded bg-white px-1.5 py-1 font-mono text-xs">meta[&quot;idempotency-key&quot;]</code> on every state-changing call. Tokens from your issuer are accepted only together with your API key.
            </p>
          </div>
        ) : null}
      </div>

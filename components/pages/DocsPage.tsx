          ...
          <div className="space-y-6">
            <SurfaceCard className="p-5">
              ...
              <div className="mt-4 rounded-2xl border border-[var(--portal-border)] bg-[var(--portal-surface-muted)] px-4 py-4 text-sm leading-6 text-[var(--portal-fg-muted)]">
                ...
                <p className="mt-1">
                  Pick one MCP door and do the whole journey there. On the UCP door, <code className="rounded bg-white px-1.5 py-1 font-mono text-xs text-[var(--portal-fg)]">search_catalog</code> and <code className="rounded bg-white px-1.5 py-1 font-mono text-xs text-[var(--portal-fg)]">get_product</code> and then <code className="rounded bg-white px-1.5 py-1 font-mono text-xs text-[var(--portal-fg)]">create_checkout</code> → <code className="rounded bg-white px-1.5 py-1 font-mono text-xs text-[var(--portal-fg)]">update_checkout</code> → <code className="rounded bg-white px-1.5 py-1 font-mono text-xs text-[var(--portal-fg)]">complete_checkout</code> are all calls to <code className="rounded bg-white px-1.5 py-1 font-mono text-xs text-[var(--portal-fg)]">{UCP_RESOURCE}</code>. The REST purchase lane (cart/validate → quotes/preview → checkout/intents) is a separate flow with its own quotes; its ids are not interchangeable with MCP checkout ids. Discovery works on your key alone; checkout also needs a verified end user (below).
                </p>
              </div>
            </SurfaceCard>
            ...
            <SurfaceCard className="p-5">
              ...
              <div className="mt-5 grid gap-4 xl:grid-cols-3">
                ...
                <div className="space-y-3">
                  ...
                  <p className="text-sm leading-6 text-[var(--portal-fg-muted)]">
                    Two things matter beyond the token: the <code className="rounded bg-white px-1.5 py-1 font-mono text-xs text-[var(--portal-fg)]">Mcp-Session-Id</code> header, which must stay the same across create → update → complete for one checkout (used when the access token carries no session claim; on the own-users path below the header is not read at all), and a client-generated <code className="rounded bg-white px-1.5 py-1 font-mono text-xs text-[var(--portal-fg)]">meta[&quot;idempotency-key&quot;]</code> on every state-changing call (Pivota never generates one — that is what makes a retry return the original result instead of a second quote). A bearer token gets you quote, update and read; <em>completing a charge</em> additionally requires payment authorization.
                  </p>
                </div>
              </div>
            ...
            <SurfaceCard className="p-5">
              ...
              <div className="mt-5 grid gap-4 xl:grid-cols-[1.2fr_0.8fr]">
                ...
                <div className="space-y-3 text-sm leading-6 text-[var(--portal-fg-muted)]">
                  <p>
                    The token must carry <code className="rounded bg-white px-1.5 py-1 font-mono text-xs text-[var(--portal-fg)]">iss</code> (exactly as registered), <code className="rounded bg-white px-1.5 py-1 font-mono text-xs text-[var(--portal-fg)]">sub</code>, <code className="rounded bg-white px-1.5 py-1 font-mono text-xs text-[var(--portal-fg)]">aud</code> (the audience you registered under Settings → Buyer identity; it defaults to <code className="rounded bg-white px-1.5 py-1 font-mono text-xs text-[var(--portal-fg)]">{UCP_RESOURCE}</code>, and the same token is accepted on both MCP doors), <code className="rounded bg-white px-1.5 py-1 font-mono text-xs text-[var(--portal-fg)]">exp</code>, <code className="rounded bg-white px-1.5 py-1 font-mono text-xs text-[var(--portal-fg)]">iat</code>, and a <code className="rounded bg-white px-1.5 py-1 font-mono text-xs text-[var(--portal-fg)]">kid</code> in its header that is present in your JWKS.
                  </p>
                  <p>
                    <span className="font-semibold text-[var(--portal-fg)]">Session claim (required for checkout):</span> put a stable id for this checkout in <code className="rounded bg-white px-1.5 py-1 font-mono text-xs text-[var(--portal-fg)]">sid</code> (or <code className="rounded bg-white px-1.5 py-1 font-mono text-xs text-[var(--portal-fg)]">session_id</code> / <code className="rounded bg-white px-1.5 py-1 font-mono text-xs text-[var(--portal-fg)]">acp_session_id</code>; if more than one is present, <code className="rounded bg-white px-1.5 py-1 font-mono text-xs text-[var(--portal-fg)]">acp_session_id</code> wins) and keep it identical on create, update, get and complete. Without it the call is refused with USER_AUTH_REQUIRED. On this path the <code className="rounded bg-white px-1.5 py-1 font-mono text-xs text-[var(--portal-fg)]">Mcp-Session-Id</code> header is not read.
                  </p>
                  <p>
